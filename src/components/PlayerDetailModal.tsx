...
export function PlayerDetailModal() {
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', inset: 0, background: 'rgba(3,6,4,.88)', backdropFilter: 'blur(4px)', zIndex: 55, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '32px 20px', overflowY: 'auto' }}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ width: '100%', maxWidth: 600, position: 'relative', background: 'linear-gradient(145deg, #1a1e1b 0%, #0b110c 55%, #111813 100%)', border: `1px solid rgba(${rgb},.22)`, borderRadius: 14, boxShadow: `0 32px 80px rgba(0,0,0,.75), 0 0 60px rgba(${rgb},.06)` }}
      >
        {/* Accent stripe */}
        ...
        <div style={{ padding: '24px 24px 24px 32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 18 }}>
            <div style={{
              ...
              font: "500 18px 'Lexend Deca'", color: avatarColor,
              boxShadow: `0 0 18px ${avatarColor}28`,
            }}>{initials}</div>

            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ font: "500 18px/1 'Lexend Deca'", color: 'var(--ink)', letterSpacing: '.02em' }}>{o.name.toUpperCase()}</span>
              ...
              {clanDoc && <div style={{ marginTop: 4, font: "500 9px 'Roboto Mono'", color: P.a, letterSpacing: '.04em' }}>{clanDoc.tag} {clanDoc.name}</div>}
          ...
          {/* Stats strip */}
          <div style={{ height: 1, background: `rgba(${rgb},.1)`, marginBottom: 16 }} />
          ...
          {/* Skill radar + bars (full width, side by side) */}
          <div style={{ marginBottom: 20, display: 'flex', gap: 18, alignItems: 'flex-start' }}>
            <div style={{ flexShrink: 0 }}>
              <div style={{ font: "500 8px 'Roboto Mono'", letterSpacing: '.18em', color: 'var(--mut)', marginBottom: 10 }}>SKILL RADAR</div>
              <Radar skillXp={o.skillXp} palette={o.palette} size={150} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ font: "500 8px 'Roboto Mono'", letterSpacing: '.18em', color: 'var(--mut)', marginBottom: 10 }}>SKILL AXES</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {SKILL_DEFS.map(s => {
                  const v  = xpToScore(o.skillXp[s.id] || 0)
                  ...
                  return (
                    <div key={s.id}>
                      ...
                      <div style={{ height: 3, background: 'rgba(255,255,255,.06)', borderRadius: 2 }}>
                        <div style={{ width: v + '%', height: '100%', background: `linear-gradient(90deg, ${ac}bb, ${ac})`, borderRadius: 2 }} />
                      </div>
                    </div>
                  )
                })}
          ...
          <div style={{ height: 1, background: `rgba(${rgb},.08)`, marginBottom: 18 }} />

          {/* Intel: 2-column grid using full width */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>

            {/* Left: Codeforces + Village */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>

              {/* Codeforces */}
              {(o.cf?.rating || o.cf?.handle) && (
                <div>
                  <div style={{ font: "500 8px 'Roboto Mono'", letterSpacing: '.18em', color: 'var(--mut)', marginBottom: 8 }}>CODEFORCES{o.cf.handle ? ' · @' + o.cf.handle : ''}</div>
                  {o.cf.rating ? (
                    ...
                    <div style={{ font: "400 9px 'Roboto Mono'", color: 'var(--dim2)' }}>not synced</div>
                  )}
                </div>
              )}
              ...
              {o.currentTarget && (
                <div>
                  <div style={{ font: "500 8px 'Roboto Mono'", letterSpacing: '.18em', color: 'var(--mut)', marginBottom: 6 }}>CURRENT TARGET</div>
                  <div style={{ font: "500 11px 'Lexend Deca'", color: P.a, lineHeight: 1.4 }}>{o.currentTarget}</div>
                </div>
              )}
            </div>

            {/* Right: Platforms + Arena + Workout */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    {o.gh?.handle && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', background: 'rgba(255,255,255,.04)', borderRadius: 5, padding: '7px 9px' }}>
                        <div>
                          ...
                          <div style={{ font: "500 11px/1 'Roboto Mono'", color: P.a2 }}>@{o.gh.handle}</div>
                        </div>
                        {o.gh.public_repos != null && (
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ font: "400 7px 'Roboto Mono'", color: 'var(--mut)', marginBottom: 2 }}>REPOS</div>
                            <div style={{ font: "500 13px/1 'Roboto Mono'", color: P.a }}>{o.gh.public_repos}</div>
                          </div>
                        )}
                      </div>
                    )}
                    {o.mt?.handle && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', background: 'rgba(255,255,255,.04)', borderRadius: 5, padding: '7px 9px' }}>
                        <div>
                          ...
                          <div style={{ font: "500 11px/1 'Roboto Mono'", color: P.a2 }}>@{o.mt.handle}</div>
                        </div>
                        {o.mt.pb60 != null && (
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ font: "400 7px 'Roboto Mono'", color: 'var(--mut)', marginBottom: 2 }}>PB 60s</div>
                            <div style={{ font: "500 13px/1 'Roboto Mono'", color: P.a }}>{o.mt.pb60} wpm</div>
                          </div>
                        )}
                      </div>
                    )}
                    {o.cc?.handle && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', background: 'rgba(255,255,255,.04)', borderRadius: 5, padding: '7px 9px' }}>
                        <div>
                          ...
                          <div style={{ font: "500 11px/1 'Roboto Mono'", color: P.a2 }}>@{o.cc.handle}</div>
                        </div>
                        {o.cc.rating && (
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ font: "400 7px 'Roboto Mono'", color: 'var(--mut)', marginBottom: 2 }}>{o.cc.stars}★</div>
                            <div style={{ font: "500 13px/1 'Roboto Mono'", color: P.a }}>{o.cc.rating}</div>
                          </div>
                        )}
                      </div>
                    )}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <div style={{ flex: 1, background: 'rgba(255,255,255,.04)', borderRadius: 6, padding: '10px 12px' }}>
                  <div style={{ font: "400 7px 'Roboto Mono'", color: 'var(--mut)', marginBottom: 4, letterSpacing: '.1em' }}>ARENA</div>
                  <div style={{ font: "500 22px/1 'Roboto Mono'", color: P.a2 }}>{arenaCount}</div>
                  <div style={{ font: "400 7px 'Roboto Mono'", color: 'var(--dim2)', marginTop: 3 }}>problems</div>
                </div>
                {o.weekWorkout && (
                  <div style={{ flex: 1, background: 'rgba(255,255,255,.04)', borderRadius: 6, padding: '10px 12px' }}>
                    <div style={{ font: "400 7px 'Roboto Mono'", color: 'var(--mut)', marginBottom: 4, letterSpacing: '.1em' }}>WORKOUT / WEEK</div>
                    <div style={{ font: "500 22px/1 'Roboto Mono'", color: P.a2 }}>{o.weekWorkout.sessions}</div>
                    <div style={{ font: "400 7px 'Roboto Mono'", color: 'var(--dim2)', marginTop: 3 }}>sessions</div>
                  </div>
                )}
              </div>
            </div>
          ...
          {o.recentActivity && o.recentActivity.length > 0 && (
            <div style={{ marginTop: 18 }}>
              <div style={{ height: 1, background: `rgba(${rgb},.08)`, marginBottom: 14 }} />
          ...
          {clanDoc && !isYou && data.clanId !== o.clanId && (
            <div style={{ marginTop: 20, borderTop: `1px solid rgba(${rgb},.1)`, paddingTop: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
              <div>
