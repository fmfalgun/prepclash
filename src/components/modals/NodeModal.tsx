...
export function NodeModal() {
  ...
  return (
    <ModalShell kicker="VILLAGE NODE" title={n.name}>
      <div style={{ font: "400 11px 'Lexend Deca'", color: 'var(--txt)', lineHeight: 1.5, marginBottom: 14 }}>
        {n.desc}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={label} style={{ flex: 1, background: 'rgba(var(--rgb),.05)', borderRadius: 4, padding: 10, textAlign: 'center' }}>
            ...
            <div style={{ font: "500 13px 'Lexend Deca'", color: accent ? 'var(--a2)' : 'var(--ink)', marginTop: 3 }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      {cleared && (
        <div style={{ background: 'rgba(var(--a2rgb),.08)', border: '1px solid rgba(var(--a2rgb),.25)', borderRadius: 5, padding: 14, textAlign: 'center' }}>
          <div style={{ font: "500 14px 'Lexend Deca'", color: 'var(--a2)', letterSpacing: '.05em' }}>✓ NODE CLEARED</div>
          <div style={{ font: "400 10px 'Roboto Mono'", color: 'var(--mut)', marginTop: 6, wordBreak: 'break-all' }}>
            PROOF: <a href={data.village[n.id].proof} target="_blank" rel="noopener" style={{ color: 'var(--a)' }}>{data.village[n.id].proof}</a>
          </div>
          <button
            onClick={unclearNode}
            style={{ cursor: 'pointer', marginTop: 12, border: '1px solid rgba(255,90,90,.3)', background: 'transparent', color: '#c0746f', font: "400 10px 'Roboto Mono'", letterSpacing: '.1em', padding: '8px 14px', borderRadius: 4 }}
      ...
      {!cleared && !unlocked && (
        <div style={{ background: 'rgba(255,90,90,.06)', border: '1px solid rgba(255,90,90,.2)', borderRadius: 5, padding: 16, textAlign: 'center' }}>
          <div style={{ font: "500 13px 'Lexend Deca'", color: '#c0746f', letterSpacing: '.05em' }}>🔒 LOCKED</div>
          <div style={{ font: "400 10px 'Roboto Mono'", color: 'var(--mut)', marginTop: 6, lineHeight: 1.5 }}>
