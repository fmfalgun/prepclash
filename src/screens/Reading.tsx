...
export function Reading() {
  ...
  // reading logs with structured data
  ...
  // day streak (any reading activity)
      ...
      <div style={card}>
        ...
        <div className="lib-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 20px' }}>
          {defs.filter(def => {
            ...
            const skillDef = SKILL_DEFS.find(s => s.id === (def as unknown as { skill?: string }).skill)
            const catName  = skillDef?.name.split(' ')[0].toLowerCase() || ''
            return (
              <div key={def.id} onClick={() => setModal('reading')} style={{ cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5 }}>
                  <span style={{ font: "400 10px 'Roboto Mono'", color: 'var(--mut)' }}>{def.unit} · {catName}</span>
              ...
              : unitArr.map(([u, tot]) => (
                  <div key={u}>
                    ...
                    <div style={{ height: 6, background: 'var(--cardHi)', borderRadius: 3, overflow: 'hidden' }}>
                      <div style={{ height: '100%', borderRadius: 3, background: 'var(--a)', width: Math.round(tot / uMax * 100) + '%' }} />
                    </div>
                  </div>
                ))
            }
